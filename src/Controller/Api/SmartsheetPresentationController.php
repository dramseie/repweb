...
class SmartsheetPresentationController extends AbstractController
{
    ...
    public function exportPresentationXlsx(\Symfony\Component\HttpFoundation\Request $request): BinaryFileResponse
    {
        ...
        $headerStyle = [
            ...
            'borders' => [
                'allBorders' => [
                    'borderStyle' => SheetBorder::BORDER_THIN,
                    'color' => ['rgb' => 'D1D5DB'],
                ],
            ],
        ];
        ...
        $addSheet = static function (
            ...
            if ($rowNumber > 2) {
                $tableRange = 'A1:' . $lastCol . ($rowNumber - 1);
                $table = new Table($tableRange, $tableStyleName);
    ...
    public function trendSeries(Request $request): JsonResponse
    {
        ...
        $country = trim((string) $request->query->get('country', ''));
        $sitesParam = $request->query->get('sites', []);
        $tasksParam = $request->query->get('tasks', []);
